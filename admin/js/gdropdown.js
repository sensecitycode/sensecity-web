...
function dropDownControl(options) {
    ...
    container.style = "min-width: 200px;width:15%";
    options.gmap.controls[options.position].push(container);
    
    google.maps.event.addDomListener(control, 'click', function () {
        $('#myddOptsDiv').css("width","150%");
        if (document.getElementById('myddOptsDiv').style.display == 'block') {
            $('#myddOptsDiv').css("display", "none");
        } else {
            $('#myddOptsDiv').css("display", "block");
        }

    });
    
    google.maps.event.addDomListener(container, 'mouseleave', function () {
        $('#myddOptsDiv').css("width","150%");
        if (document.getElementById('myddOptsDiv').style.display == 'block') {
            $('#myddOptsDiv').css("display", "none");
        }
